...
static int readdir_multi_head_4_newcache(const char* path, const S3ObjList& head, void* buf, fuse_fill_dir_t filler);
...
static int s3fs_symlink(const char* _from, const char* _to)
{
    ...
    {   // scope for AutoFdEntity
        ...
        ssize_t ressize;
        if(from_size != (ressize = ent->Write(autoent.GetPseudoFd(), strFrom.c_str(), 0, from_size, true))){
            if(ressize < 0){
        ...
        // upload
        if(0 != (result = ent->Flush(autoent.GetPseudoFd(), AutoLock::NONE, true, true))){
...
static int s3fs_truncate(const char* _path, off_t size)
{
    ...
    if(0 == (result = get_object_attribute(path, nullptr, &meta))){
        ...
        if(use_newcache){
            int res = accessor->Truncate(path, size);
            if(res) return res;
        }
        ent->UpdateCtime();
...
static int s3fs_read(const char* _path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
    ...
    S3FS_PRN_WARN("[path=%s][size=%zu][offset=%lld][pseudo_fd=%llu]", path, size, static_cast<long long>(offset), (unsigned long long)(fi->fh));
...
static int s3fs_write(const char* _path, const char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
    ...
    S3FS_PRN_WARN("[path=%s][size=%zu][offset=%lld][pseudo_fd=%llu]", path, size, static_cast<long long int>(offset), (unsigned long long)(fi->fh));
...
{
    if(use_newcache && accessor->UseGlobalCache()){
        return readdir_multi_head_4_newcache(path, head, buf, filler);
    }
...
static int readdir_multi_head_4_newcache(const char* path, const S3ObjList& head, void* buf, fuse_fill_dir_t filler)
{
    ...
    for(s3obj_list_t::iterator iter = headlist.begin(); headlist.end() != iter; ++iter){
        ...
        fs.push_back(folly::via(accessor->GetExecutor(), [disppath, &syncfiller, &notfound_param]() {
            size_t realSize = 0;
            std::map<std::string, std::string> headers;
            headers_t meta;
            int res = accessor->Head(get_realpath(disppath.c_str()), realSize, headers);
            if(0 == res){
                headers["Content-Length"] = std::to_string(realSize);
                for(auto& it : headers) {
                    meta.insert(std::make_pair(it.first, it.second));
                }
                if(!StatCache::getStatCacheData()->AddStat(disppath, meta)){
                    S3FS_PRN_ERR("failed adding stat cache [path=%s]", disppath.c_str());
                    meta.clear();
                    return -EIO;
                }
                meta.clear();  // ps: it must be released here.
                ...
                struct stat st;
                if(StatCache::getStatCacheData()->GetStat(disppath, &st)){
                    syncfiller.Fill(bpath.c_str(), &st, 0);
                }else{
                    S3FS_PRN_INFO2("Could not find %s file in stat cache.", disppath.c_str());
                    syncfiller.Fill(bpath.c_str(), nullptr, 0);
                }
            }else if(-ENOENT == res) {  // notfound
                notfound_param.notfound_list.push_back(disppath);
                res = 0;
            }else{
                 S3FS_PRN_ERR("File head failed [path=%s][errno=%d]", disppath.c_str(), res);
            }
            return res;
        }));
    }
    ...
    if(fs.size()){
        auto f = collectAll(fs.begin(), fs.end()).via(accessor->GetExecutor()).thenValue([](
                    std::vector<folly::Try<int>, std::allocator<folly::Try<int>>>&& tups) {
                for(const auto& t : tups){
                    if (0 != t.value()) return t.value();
                }
                return 0;
            });
        f.wait();
        result = f.value();
        FUSE_CTX_INFO("multi request [path=%s][list=%zu][result=%d]", path, headlist.size(), result);

        // If result is -EIO, it is something error occurred.
        // This case includes that the object is encrypting(SSE) and s3fs does not have keys.
        // So s3fs set result to 0 in order to continue the process.
        if(-EIO == result){
            S3FS_PRN_WARN("error occurred in multi request, but continue... [path=%s][list=%zu][errno=%d]", path, headlist.size(), result);
            result = 0;
        }else if(0 != result){
            S3FS_PRN_ERR("error occurred in multi request [path=%s][list=%zu][errno=%d]", path, headlist.size(), result);
            return result;
        }
    }
    headlist.clear();
...
// Also it causes a conflict with WinFsp's validation, so disabling it.
        ...
        else if(is_prefix(arg, "newcache_conf=")){
            newcache_conf = std::string(strchr(arg, '=') + sizeof(char));
            if(!newcache_conf.empty()) use_newcache = true;
            return 0;
        }
    }
    return 1;
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    s3fs_oper.getattr     = s3fs_getattr;  // stat()
    ...
    s3fs_oper.readdir     = s3fs_readdir;  // list
