...
void FdManager::ReleaseCache(const std::string &avoid_path, off_t size, const std::string &dir)
{
    DIR*           dp;
    struct dirent* dent;
    std::string    abs_path = cache_dir + "/" + S3fsCred::GetBucket() + dir;
    ...
    for(dent = readdir(dp); dent; dent = readdir(dp)){
        if(GetFreeDiskSpace(nullptr) >= size)
            return;
        ...
        std::string next_path = dir + "/" + dent->d_name;

        if(next_path == avoid_path)
            continue;

        if(S_ISDIR(st.st_mode)){
            ReleaseCache(avoid_path, size, next_path);
        }else{
            AutoLock auto_lock(&FdManager::fd_manager_lock, AutoLock::NO_WAIT);
            if (!auto_lock.isLockAcquired()) {
                S3FS_PRN_INFO("could not get fd_manager_lock when clean up file(%s), then skip it.", next_path.c_str());
...
bool FdManager::EnsureDiskSpaceUsable(const std::string &avoid_path, off_t size)
{
    if(GetFreeDiskSpace(nullptr) >= size)
        return true;

    ReleaseCache(avoid_path, size, "");

    return GetFreeDiskSpace(nullptr) >= size;
}
